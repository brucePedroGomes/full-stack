...
import { getTasks, SessionExpiredError } from '@/lib/api'
import App from './App'
...
vi.mock('@/lib/api', async (importOriginal) => {
  ...
  return {
    ...actual,
    signIn: vi.fn(async () => ({ access: 'test-access', refresh: 'test-refresh' })),
...
test('returns to login when the session expires', async () => {
  /** Checks that an expired refresh token clears the signed-in page. */
